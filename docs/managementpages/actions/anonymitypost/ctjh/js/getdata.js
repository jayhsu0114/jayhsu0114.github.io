...
async function fetchData() {
    try {
        const response = await fetch('https://google-sheets-proxy-mk66ircp2a-uc.a.run.app/ctjh-update-status');
        if (!response.ok) {
            throw new Error('Network response was not ok ' + response.statusText);
        }
...
function updatePostGroup() {
    ...
    const postCode = sessionStorage.getItem(postKey);
    if (postCode) {
        // 處理 postCode，去掉 "CTJH" 的開頭，將剩下的數字 +9，再加回 "CTJH"
        let postCodeEnd;
        if (postCode.startsWith('CTJH')) {
            const numericPart = parseInt(postCode.substring(4), 10);
            postCodeEnd = 'CTJH' + (numericPart + 9);
        } else {
            postCodeEnd = postCode; // 如果沒有 "CTJH" 開頭，保持不變
        }
        ...
        for (let i = 1; i <= 10; i++) {
            const currentPostCode = 'CTJH' + (parseInt(postCode.substring(4), 10) + (i - 1));
